...
static VALKEY_CONTAINERS: LazyLock<Mutex<HashMap<String, &'static OnceCell<ContainerAsync<Valkey>>>>> =
...
pub async fn valkey_url(container_name: &str) -> String {
    ...
    format!("valkey://127.0.0.1:{port}/{db}")
}

/// Returns the cached container instance for the given name, starting it if necessary.
pub async fn get_container(container_name: &str) -> &'static ContainerAsync<Valkey> {
    let cell = {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_valkey_url_includes_db() {
        let name = "test-valkey-url-db";
        let url = valkey_url(name).await;
        // url should be valkey://127.0.0.1:PORT/0
        assert!(url.contains("/0"));
        
        let url2 = valkey_url(name).await;
